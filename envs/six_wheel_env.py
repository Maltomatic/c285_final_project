...
from envs.env_configs import _ACTION_DIM, _OBS_DIM, _ACTION_CLIP, _OBS_STACK, _FRAME_SKIP,\
    ...
    EVAL_EPISODES, FAULT_STEPS, GYM_RANDOM
...
class SixWheelEnv(gym.Env):
    ...
    def _inject_fault_eval(self) -> None:
        ...
        if env_config.SAME_SIDE_FAULT and n > 1:
            ...
            idxs = self._rng_choice(candidates, size=n, replace=False)
        else:
            idxs = self._rng_choice(6, size=env_config.NUM_FAULT_WHEELS, replace=False)
